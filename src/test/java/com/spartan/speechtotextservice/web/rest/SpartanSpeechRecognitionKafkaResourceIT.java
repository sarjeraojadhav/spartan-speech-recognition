package com.spartan.speechtotextservice.web.rest;
...
import com.spartan.speechtotextservice.IntegrationTest;
import com.spartan.speechtotextservice.config.EmbeddedKafka;
import com.spartan.speechtotextservice.config.KafkaSseConsumer;
import com.spartan.speechtotextservice.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class SpartanSpeechRecognitionKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/spartan-speech-recognition-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/spartan-speech-recognition-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/spartan-speech-recognition-kafka/unregister"));
                return;
            }
        }
